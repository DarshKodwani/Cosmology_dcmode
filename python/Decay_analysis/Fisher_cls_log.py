...
dclfull = np.load("Fish_data/dcfull_%i_log.npy" %numks) #[numks, nell, (TT,EE,TE)]
clmat = np.load("Fish_data/clfull_%i_log.npy" %numks) #[(TT,EE,TE), nell]
...
for i in np.arange(numks) : 
    ...
    for j in np.arange(numks) : 
        ...
        for l in np.arange(len(clmat[0,0])) : 
            ...
            full_fish[i,j] += fish_sum

np.save("Fish_data/fish_%i_log" %numks, full_fish)
