...
om_gamma = 5.050601e-05
a = one_k['a']
R = 0.75*M.Omega_b()/om_gamma*a
zero_point = -(1.+R)*psi
...
background_k_over_aH = k/background['H [1/Mpc]']*(1.+background['z']) #k/aH, relative size of mode to comoving horizon
background_k_rs = k*background['comov.snd.hrz.'] #k*rs
background_rhom_over_rhor = (background['(.)rho_b']+background['(.)rho_cdm'])/(background['(.)rho_g']+background['(.)rho_ur']) #rho_m/rho_r to find time of equality

#Interpolating functions

tau_at_k_over_aH = interp1d(background_k_over_aH, background_tau)
tau_at_k_rs = interp1d(background_k_rs, background_tau)
tau_at_rho_m_over_r = interp1d(background_rhom_over_rhor, background_tau)

#Get the times of hubble crossing, peaks in oscillations, matter_radiation equality
...
plt.legend(loc='right',bbox_to_anchor=(1.4, 0.5))
plt.savefig('one_k_decay.pdf', bbox_inches = 'tight')
