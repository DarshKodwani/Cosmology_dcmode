...
np.save("Fish_data/clfull_%i_lin" %numks, clfull)
...
for i in np.arange(numks) : 
    cosmop = Class(); cosmop.set(params) ; cosmop.set({'k_amp' : ks_arr[i], 'dAmp_k' : dAMP*(1.+ 0.1)}) ; cosmop.compute() ; cosmop.empty()
    cosmom = Class(); cosmom.set(params) ; cosmom.set({'k_amp' : ks_arr[i], 'dAmp_k' : dAMP*(1 - 0.1)}) ; cosmom.compute() ; cosmom.empty()
...
np.save("Fish_data/dcfull_%i_lin" %numks, dcl)
